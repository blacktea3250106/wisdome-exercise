...
為了更具挑戰性，也具有解析`104至112學年度學科能力測驗－英文`的試題的相容性。
...
- **挑戰**：正規表達式誤讀content文字，使資料格式錯誤。
- **解決方法**：在解析questions前，移除content文字。
...
- **解決方法**：
  1. 初步提取：先使用 `(\d+)[.](.*?)(?=\n\d+\.|\n-|$)` 提取題目的id和其內容。
  2. 細部解析：將提取的內容透過迴圈再進行細部分析，拆解出text和options。

---

### 4. Content 的換行問題
- **挑戰**：content文字中有正常和格式限制的非自然的換行，
